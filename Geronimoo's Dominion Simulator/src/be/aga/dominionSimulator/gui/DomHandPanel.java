...
public class DomHandPanel extends JPanel {
	...
	public DomHandPanel(String string, DomGameFrame aGameFrame) {
...
//		}
		...
		if (aCards.size()>FIRST_TRESHOLD){
			...
			GridBagConstraints theCons = DomGui.getGridBagConstraints(2);
			while (i<aCards.size()){
				DomCardLabel label = new DomCardLabel(aCards.get(i).getName(), myGameFrame);
				label.addMouseListener(new HandCardListener(aCards.get(i), myGameFrame));
				i++;
				add(label, theCons);
				theCons.gridx++;
