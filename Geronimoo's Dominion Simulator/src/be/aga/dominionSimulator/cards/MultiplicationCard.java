...
public class MultiplicationCard extends DomCard {
	...
	public DomCard getCardToMultiply( ) {
		if (owner instanceof DomHumanPlayer) {
			ArrayList<DomCard> actions = owner.getCardsFromHand(DomCardType.Action);
			if (actions.isEmpty())
				return null;
			// TODO Throne Room is optional
			String c = ((DomHumanPlayer) owner).chooseOption("Multiply an actions?", "Choose action", "Don't choose");
			if (c.startsWith("Choose"))
				return ((DomHumanPlayer) owner).chooseExactlyNCardsFromList(1, actions, "Choose action card").get(0);
			else
				return null;
		} else {
