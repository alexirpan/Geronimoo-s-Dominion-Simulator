...
public class ForgeCard extends DomCard {
    ...
    public void play() {
    	if (owner instanceof DomHumanPlayer) {
    		DomCost totalCost = DomCost.ZERO;
    		ArrayList<DomCard> cardsToTrash = ((DomHumanPlayer) owner).chooseUpToNCardsFromList(owner.getCardsInHand().size(), owner.getCardsInHand(), "Forge - trash any number of cards");
    		for (DomCard theCard : cardsToTrash) {
    			totalCost = totalCost.add(new DomCost(theCard.getName().getCost(owner.getCurrentGame()).getCoins(), 0) );
    			owner.trash(owner.removeCardFromHand(theCard));
    		}
    		((DomHumanPlayer) owner).gainCardOfExactCost(totalCost, "Forge - gain card", false);
    	} else {
